...
int    max(const int    *V, int len){return V[ argmax(V,len) ]; }
double min(const double *V, int len){return V[ argmin(V,len) ]; }
int    min(const int    *V, int len){return V[ argmin(V,len) ]; }
...
//================================================================================
void HardDecision(int **V, const double **P, int Ns, int Nu, int Q){
  assert( Ns>0 && Nu>0 && Q>0);
  for(int i=0; i<Ns; i++){
    for(int j=0; j<Nu; j++){
      V[i][j] = argmax( P[i*Nu+j], Q );
    } // for j
  } // for i
}
...
//================================================================================
int HammingDist(const int **V0, const int **V1, int l0, int l1){
  assert(l0>0 && l1>0);
  int cnt=0;
  for(int i=0; i<l0; i++){
    for(int j=0; j<l1; j++){
      if(V0[i][j] != V1[i][j]) cnt++;
    } // for j
  } // for i
  return cnt;
}
