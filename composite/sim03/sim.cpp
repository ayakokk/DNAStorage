...
long PdistArgMaxProb(const double *P, int Q, int LS);
...
int main(int argc, char *argv[]){
  ...
  printf("# QA=%d Ns=%d Nu=%d Nb=%d (Pi,Pd,Ps)=(%e,%e,%e) Nseq=%d Ndi=%d OutListSize=%d [%d]\n",
	 QA,Ns,Nu,Nb,Pi,Pd,Ps,Nseq,Ndi,OutListSize,seed);
  printf("# ICB:   %s\n",fncb);
  printf("# Const: %s\n",fnconst);
  printf("# EncCM: %s\n",fncm);
  ...
  class ChannelMatrix *DCMa = new class ChannelMatrix( QA, QA *OutListSize );
  class ChannelMatrix *DCMb = new class ChannelMatrix( QB2,QB2*OutListSize );
...
void OutputConv(long *DWL, const double **P, int Ns, int Q){
  for(int i=0;i<Ns;i++){
    ...
    DWL[i] = PdistArgMaxProb(P[i], Q, OutListSize);
    //printf("%03d: %ld\n",i,DWL[i]);
    //PrintVect(P[i],Q," ","\n");
  } // for i
}
...
void OutputConv(long **DWL, const double **P, int Ns, int Nu, int Q){
  for(int i=0; i<Ns; i++){
    for(int j=0; j<Nu; j++){
      ...
      DWL[i][j] = PdistArgMaxProb( P[i*Nu+j], Q, OutListSize );
    } // for j
  } // for i
}
...
long PdistArgMaxProb(const double *P, int Q, int LS){
  assert(Q>0 && LS>0);
  long val = argmax(P, Q);
  int  pbi = (int)floor( P[val]*(double)LS );
  if( pbi==LS ) pbi--;  // p=1.0
  assert( pbi>=0 && pbi<LS );
  return pbi*Q + val;
}
