...
double CSFBAdec::max(double a, double b){return (a>b)? a : b;}
double CSFBAdec::min(double a, double b){return (a<b)? a : b;}
...
void CSFBAdec::SetGX(){
  ...
  for(int ly=0;ly<=Nu*2;ly++){
    if(ly<Nu2min || ly>Nu2max){
      ...
      if(Pi==0.0 && ly>Nu) GX[ly][0][0] = 0.0;
      if(Pd==0.0 && ly<Nu) GX[ly][0][0] = 0.0;
      //printf("GX[%d]=%e\n",ly,GX[ly][0][0]);
    } else {
      // exact
      ly2p = (long)pow(2,ly);
...
double CSFBAdec::CalcPyx(long y, long x, int ly, int lx){
  ...
  if(ly==0) return (Pd==0.0)? 0.0 : pow(Pd,lx);
  long   x1,y1;
  double ret,qt,qi,qd;
...
//================================================================================
CSFBAdec::CSFBAdec(int _Nb, double _Pi, double _Pd, double _Ps, int _Dmin, int _Dmax, 
		   class InnerCodebook *_ICB, class ChannelMatrix *_ECM, int _Nseq, int _NumItr){
  ICB  = _ICB;
  ECM  = _ECM;
  Nseq = _Nseq;
  NumItr = _NumItr;
...
//================================================================================
void CSFBAdec::Get_PFB(double ***PrD){
  for(int jdx=0; jdx<Nseq; jdx++){
    for(int idx=0; idx<Ns+1; idx++){
      for(int d=0; d<Drng; d++){
	PrD[jdx][idx][d] = PF[jdx][idx][d] * PB[jdx][idx][d];
      } // for d
      normalize(PrD[jdx][idx], Drng);
    } // for idx
  } // for jdx
}

//================================================================================
void CSFBAdec::Get_PO( double **Pout){
  for(int i=0; i<Ns; i++) memcpy( Pout[i], PO[i], sizeof(double)*Q );
}

//================================================================================
int    CSFBAdec::Get_Ns(){  return Ns;}
int    CSFBAdec::Get_Nu(){  return Nu;}
int    CSFBAdec::Get_Nb(){  return Nb;}
int    CSFBAdec::Get_Nseq(){return Nseq;}
int    CSFBAdec::Get_Q(){   return Q;}
int    CSFBAdec::Get_Dmin(){return Dmin;}
int    CSFBAdec::Get_Dmax(){return Dmax;}
int    CSFBAdec::Get_Drng(){return Drng;}
double CSFBAdec::Get_Pi(){  return Pi;}
double CSFBAdec::Get_Pd(){  return Pd;}
double CSFBAdec::Get_Ps(){  return Ps;}
double CSFBAdec::Get_Pt(){  return Pt;}
