...
//================================================================================
// 事前情報が与えられない場合のために、デフォルトで一様な事前確率を設定し、主要なデコードロジックに渡す
void SLFBAdec::Decode(double **Pout, const unsigned char *RW, int Nb2, const int *dbgIW){
  double **Pin = new double * [Ns];
  // Ns行Q列の行列を生成
  // 一様分布での初期化: 各情報シンボル位置 i における各符号語 x の事前確率 Pin[i][x] を 1.0/Q に設定します。
  // これは、どの符号語も等しい確率で出現するという、最も情報量の少ない（無情報的な）事前情報を意味します。
  for(int i=0;i<Ns;i++){
    Pin[i] = new double [Q];
