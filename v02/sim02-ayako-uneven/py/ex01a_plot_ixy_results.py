...
def plot_normalized_ixy():
    ...
    # グラフの装飾
    plt.xlabel('Pi=Pd=Ps', fontsize=14)
    ...
    # PDFとして保存
    output_pdf = 'result/ex01a_normalized_ixy_plot.pdf'
    plt.savefig(output_pdf, format='pdf', dpi=300, bbox_inches='tight')
    print(f"Plot saved to: {output_pdf}")
    
    # PNGとしても保存（プレビュー用）
    output_png = 'result/ex01a_normalized_ixy_plot.png'
...
def plot_comparison_subplots():
    ...
    # PDFとして保存
    output_pdf = 'result/ex01a_comparison_subplots.pdf'
    plt.savefig(output_pdf, format='pdf', dpi=300, bbox_inches='tight')
