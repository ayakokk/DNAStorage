...
int main(int argc, char *argv[]){
  ...
  if(argc!=8){
    fprintf(stderr,"Usage: %s <Pid> <Ps> <Nseg> <InnerCB.bmat> <Gtable.bin> <lmd> <seed|-1>\n",argv[0]);
    return 1;
  }
  Pid = atof(argv[1]);
  Ps  = atof(argv[2]);
  ...
  srandom(seed);
  printf("# (Pid,Ps)=(%e,%e) Nseg=%d WinSize=%d lmd=%d [%d]\n",Pid,Ps,Nseg,WinSize,lmd,seed);
  printf("# InnerCB: %s\n",fnI);
  printf("# Gtable:  %s\n",fnG);
  ...
  ICB->Get_B(B);
  WCmax = WCmax1*nu;
  printf("# beta=%d(%d) nu=%d N=%d Dmax=%d WCmax=%d\n",beta,beta4p,nu,N,Dmax,WCmax);
  PrintVect(B,nu,"# B=","\n");
  ...
  class StatICB *STI = new class StatICB(nu,beta,B,lmd);
  ...
  for(wc=1;wc<=WCmax;wc++){
    ...
    STI->count(ICoutI,(const double **)Pout,Nseg);
