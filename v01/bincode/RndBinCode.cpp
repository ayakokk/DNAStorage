...
int main(int argc, char *argv[]){
  ...
  if(argc!=9){
    fprintf(stderr,"Usage: %s <beta> <b> <t> <Pi> <Pd> <Ps> <output.txt> <seed|-1>\n",argv[0]);
    return 1;
  }
  beta = atoi(argv[1]);
  B    = atoi(argv[2]);
  ...
  printf("# beta=%d B=%d T=%d (Pi,Pd,Ps)=(%e,%e,%e) [%d]\n",beta,B,T,Pi,Pd,Ps,seed);
  printf("# output: %s\n",fn);
  assert(beta>0);
  assert(B>=0 && B<=beta);
  assert(T>=0 && T<=beta);
  ...
  // select
  idx = random()%NumSelPtn;
  for(int i=0;i<idx;i++) CMBsel->nextV();
  CMBsel->getV(sel);
  SelectVect(V,U,sel);
  ...
  printf("prob=%e ListSize=%lu\n",prob,LS.size());
  WriteBaseFile(fn,LS,beta,B,prob,U,MC);
