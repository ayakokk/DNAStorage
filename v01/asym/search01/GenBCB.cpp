...
int    CountCW(int cb, int posL, int posR, int wt);
...
int main(int argc, char *argv[]){
  ...
  char *fnP,*fnB;
  if(argc!=7){
    fprintf(stderr,"%s <CodeParam.txt> <Lr> <Lb> <eps> <BCB.bin> <seed|-1>\n",argv[0]);
    return 1;
  } // if
  fnP  =     argv[1];
  Lr  = atoi(argv[2]);
  ...
  // ----- init -----
  class CodeParam *CP = new class CodeParam(fnP);
  beta= CP->get_beta();
  nu  = CP->get_nu();
  B   = new int [nu];
...
void UpdateBCB(int wpos, bool fpn){
  ...
  while(len>0){
    ...
    if(s>=smax){
      w = (fpn)? Wmax[cb][posL][posR] : Wmin[cb][posL][posR];
      c = CountCW(cb,posL,posR,w);
      printf(" (w=%d c=%d)\n",w,c);
      if(s>smax || c<cmin){
	smax = s;
	cmin = c;
...
int CountCW(int cb, int posL, int posR, int wt){
  ...
  assert(posL>=0 && posL<beta);
  assert(posR>=posL && posR<beta);
  assert(wt>=0 && wt<=posR-posL+1);
  int cnt=0, len=posR-posL+1;
  unsigned int u;
  unsigned char *U = new unsigned char [beta];
  ...
  for(u=0;u<beta2p;u++){
    if(BCB->getV(u,cb)==1){
      ConvIntV2(U,u,beta);
      if(HammingWeight(&U[posL],len)==wt) cnt++;
    } // if BCB
  } // for u
