...
void GenCBrec(unsigned char *CW, const int *TR0, const int *TR1,
	      class bmatrix *CB, int src, int stat, int pos, int beta, bool *SF);
...
void GenCB(class bmatrix *CB, const int *TR0, const int *TR1, int lmd, int beta){
  ...
  bool *SF = new bool[l2p];
  
  assert(CB->getM()==(int)l2p);
  assert(CB->getN()==(int)b2p);
  CB->clear();
  s=0;
  for(int i=0;i<lmd;i+=2) s |= (0x00000001<<i);
  SetVal(SF, false,l2p);
  SF[s]=true;
  w=1;
  ...
  while(1){
    w0=w;
    for(s=0;s<l2p;s++){
      if(SF[s]) GenCBrec(CW,TR0,TR1,CB,s,s,beta-1,beta,SF);
    } // for s
    w = HammingWeight(SF,l2p);
    printf("%d->%d\n",w0,w);
    PrintVect2(SF,l2p,"","\n");
    if(w==w0) break;
  } // while(1)
...
void GenCBrec(unsigned char *CW, const int *TR0, const int *TR1,
  ...
  if(pos==beta-1){
	...
	CW[pos]=0;
	GenCBrec(CW,TR0,TR1,CB,src,TR0[stat],pos-1,beta,SF);
      } // if stat%2
    } // if TR0
    if(TR1[stat]>=0){
      if(stat%2==0 || TR0[stat]<0){
	CW[pos]=1;
	GenCBrec(CW,TR0,TR1,CB,src,TR1[stat],pos-1,beta,SF);
      } // if stat%2
    } // if TR1
  } else if(pos>0){
    // --------------- pos==1...beta-2
